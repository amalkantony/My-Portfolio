/* eslint-disable @next/next/no-img-element */
"use client";
import { useRef, useEffect, useState, useCallback, useMemo } from "react";

import img from '@/assets/test3.jpg'
import gsap, { Elastic } from "gsap";



let particles :any= []


export default function Home() {
  const canvasRef = useRef<HTMLCanvasElement>(null) as any;


  const imageRef = useRef<any>(null) as any;


  const drawScene = () => { 
    let ctx = canvasRef.current.getContext("2d");
    canvasRef.current.width = imageRef.current.width*6;
    canvasRef.current.height = imageRef.current.height*6;

    ctx.drawImage(imageRef.current, 0, 0);

    const data = ctx.getImageData(0, 0, imageRef.current.width, imageRef.current.height);
 console.log("data",data)
 console.log("particles",particles)

 ctx.clearRect(0,0,canvasRef.current.width, canvasRef.current.height);

 particles = [];


 ctx.fillStyle = "red";
 for (let y = 0;y < data.height;  y++) {
  for (let x = 0; x < data.width; x++) {

    const index = (y * 4 * data.width) + (x * 4);
    const r = data.data[index];
    const g = data.data[index + 1];
    const b = data.data[index + 2];
    const a = data.data[index + 3];

    console.log("rgb",r,g,b)

    // console.log("a",a)


    if (r === 0 && g === 0 && b === 0 && a === 255 ) {
      console.log(`Black pixel found at (${x}, ${y})`);

      const particle = {
        x0: x,
        y0: y,
        color: "black",
        x1: imageRef.current.width / 2,
        y1: imageRef.current.height / 2,
        speed: Math.random() * 4 + 2
      };
      gsap.to(particle, {
        duration: particle.speed,
        x1: particle.x0,
        y1: particle.y0,
        delay: y / 30,
        ease: Elastic.easeOut,
        
      });
      particles.push(particle);
    }
  }
}

requestAnimationFrame(render);

   }

   const render = function() {
    requestAnimationFrame(render);
    let ctx = canvasRef.current.getContext("2d");
  
    ctx.clearRect(0, 0, canvasRef.current.width, canvasRef.current.height);
    for (let i = 0, j = particles.length; i < j; i++) {
      const particle = particles[i];
      ctx.fillStyle = particle.color;
      ctx.fillRect(particle.x1 * 6, particle.y1 * 6, 3,3);

      
    }
  };





  // const png = new Image();
// png.onload = drawScene;
// png.src = img;


  return (
    <main className="">
 
        <button onClick={drawScene} className="btn border border-red-500">Hello  load</button>
      <canvas ref={canvasRef} className="border border-blue-800" />
      <img ref={imageRef} className="w-10 h-10 hidden" src={img.src} alt="" />
      {/* <img ref={imageRef} src={"data:image/png;base64,"} alt="" /> */}

    </main>
  );
}
